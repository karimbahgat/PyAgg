...
if __name__ == "__main__":
    ...
    def recur(svg):
        yield svg
        for sub in svg.sub:
            for subsub in recur(sub):
                yield subsub
    
    teacher = load("tiger.svg")

    # dont constrain size
##    teacher.attr.pop("width")
##    teacher.attr.pop("height")
##    #teacher.attr["viewBox"] = "0 0 15 15"
##    
##    for svg in loopitems(teacher):
##        if svg.t == "rect":
##            svg.attr["fill"] = "rgb(222,111,111)" # orange
##        elif svg.t == "polygon":
##            svg.attr["fill"] = "rgb(111,111,111)" # grey
##            svg.attr["stroke"] = "rgb(0,0,0)" # black
##            svg.attr["stroke-width"] = 1
##        elif svg.t == "circle":
##            svg.attr["fill"] = "rgb(255,211,111)" # pink
##            svg.attr["stroke"] = "rgb(0,0,0)" # black
##            svg.attr["stroke-width"] = 1 # width is forced to 1 pixel anyway due to error
##        
##        print svg.t, svg.attr.get("fill")
##    
##    #backdrop = SVG("rect", x=0, y=0, width=100, height=100, fill="green")
##    #grouped = SVG("g", backdrop, teacher)#, fill_opacity="50%")
##
##    teacher.save("teacher4_mod.svg")

    def svg_to_image(svg, width, height):

        import pydraw

        def drawoptions_from_svg(svg):
            def get_color(color):
                if isinstance(color, (str,unicode)):
                    if color.startswith("rgb"):
                        ...
                        return rgb
            options = dict()
            fillcolor = item.attr.get("fill")
            if fillcolor:
                options["fillcolor"] = get_color(fillcolor)
            outlinecolor = item.attr.get("stroke")
            if outlinecolor:
                options["outlinecolor"] = get_color(outlinecolor)
            outlinewidth = item.attr.get("stroke-width")
            if outlinewidth:
                options["outlinewidth"] = outlinewidth
            return options

        def svg_to_crs(svg):
            viewbox = map(float,svg.attr["viewBox"].split())
            x,y,width,height = viewbox
            bbox = [x,y,x+width,y+height]
            crs = pydraw.CoordinateSystem(bbox)
            return crs

        crs = svg_to_crs(svg)
        img = pydraw.Image(width, height, crs=crs)
        for item in loopitems(svg):
            if item.t == "rect":
                x,y = map(float,(item.attr["x"],item.attr["y"]))
                _width,_height = map(float,(item.attr["width"],item.attr["height"]))
                bbox = [x,y,x+_width,y+_height]
                options = drawoptions_from_svg(svg)
                if img.coordmode:
                    outlinewidth = options.get("outlinewidth")
                    if outlinewidth: options["outlinewidth"] = outlinewidth / img.crs.xwidth * img.width
                print bbox, options
                img.drawrectangle(bbox, **options)
            elif item.t == "polygon":
                strpoints = item.attr["points"].split()
                points = [tuple(map(float,p.split(","))) for p in strpoints]
                options = drawoptions_from_svg(svg)
                if img.coordmode:
                    outlinewidth = options.get("outlinewidth")
                    # ERROR WITH POLYGON OUTLINEWIDTH IF BIGGER THAN 1
                    #if outlinewidth: options["outlinewidth"] = outlinewidth / img.crs.xwidth * img.width
                print points, options
                img.drawpolygon(points, **options)
            elif item.t == "circle":
                x,y = map(float,(item.attr["cx"],item.attr["cy"]))
                fillsize = float((item.attr["r"]))
                options = drawoptions_from_svg(svg)
                if img.coordmode:
                    fillsize = fillsize / img.crs.xwidth * img.width
                    # CIRCLE OUTLINE ERROR?
                    #outlinewidth = options.get("outlinewidth")
                    #if outlinewidth: options["outlinewidth"] = outlinewidth / img.crs.xwidth * img.width
                print x,y,fillsize, options
                img.drawcircle(x,y,fillsize,**options)
            elif item.t == "g":
                options = drawoptions_from_svg(svg)
                print options
            elif item.t == "path":
                coordstring = item.attr["d"]
                i = 0
                cur = coordstring[i]
                coords = []
                subcoords = []
                while i < len(coordstring):
                    try:
                        if cur in (" ",","):
                            # get nr
                            nr = ""
                            i += 1
                            cur = coordstring[i]
                            while cur.isdigit() or cur == ".":
                                nr += cur
                                i += 1
                                cur = coordstring[i]
                            subcoords.append(float(nr))
                        elif cur == "-":
                            # get nr
                            nr = "-"
                            i += 1
    ...
    img = svg_to_image(teacher,600,600)
    img.save("tiger.png")
    #img.view()
